    ...
    st.markdown(f"""
        <style>
            ...
            section[data-testid="stAppViewContainer"] > div:first-child > div:first-child {{
                background-color: #FFFFFF; padding: 1.5rem 2.5rem 2.5rem 2.5rem;
                border-radius: 20px; box-shadow: 0 8px 25px rgba(0,0,0,0.07);
                border: 1px solid #E0E0E0;
            }}
            h2 {{
                color: {primary_color}; font-size: 1.6rem; font-weight: 700;
                margin-top: 1rem; margin-bottom: 2rem; text-align: center;
                border-bottom: 2px solid {background_color}; padding-bottom: 0.5rem;
            }}
            h3 {{
                color: {secondary_color}; font-weight: 600; font-size: 1.1rem;
                margin-top: 1.5rem;
            }}
            .stTextInput input, .stNumberInput input {{
                background-color: transparent !important; border: none !important;
                border-bottom: 2px solid {light_gray} !important; border-radius: 0 !important;
                padding-left: 0 !important; transition: border-bottom-color 0.3s ease;
            }}
            .stTextInput input:focus, .stNumberInput input:focus {{
                border-bottom-color: {primary_color} !important; box-shadow: none !important;
            }}
            .stButton > button {{
            ...
            .stButton > button:hover {{
                background-color: {secondary_color}; transform: scale(1.02);
            }}
            div[data-baseweb="radio"] input[type="radio"] {{ display: none; }}
            div[data-baseweb="radio"] label {{
                display: block; background-color: #F0F0F0; color: {text_color};
                border-radius: 10px; padding: 0.75rem 1rem; margin: 0.25rem 0;
                cursor: pointer; border: 2px solid transparent; transition: all 0.2s ease-in-out;
                font-weight: 500; text-align: center;
            }}
            div[data-baseweb="radio"] label:hover {{ background-color: #E0E0E0; border-color: {light_gray}; }}
            div[data-baseweb="radio"] input[type="radio"]:checked + div {{
                background-color: {primary_color}; color: white;
                border-color: {secondary_color}; font-weight: 700;
            }}
...
def render_step_2():
    ...
    with st.form("step2_form"):
        ...
        back_clicked = nav_cols[0].form_submit_button("⬅️ Voltar")
        next_clicked = nav_cols[1].form_submit_button("Próximo ➡️")
        if back_clicked:
...
def render_step_3():
    ...
    # Widgets de upload ficam fora do formulário para serem lidos antes
    ...
    with st.form("step3_form"):
        ...
        back_clicked = nav_cols[0].form_submit_button("⬅️ Voltar")
        submit_clicked = nav_cols[2].form_submit_button("✔️ SUBMETER INSPEÇÃO")
...
# --- FUNÇÃO DA TELA DE SUCESSO (ATUALIZADA) ---
def render_success_step():
    # Mensagem de sucesso
    st.success("🎉 Inspeção registrada com sucesso!")
    
    # Colunas para centralizar a imagem da águia
    col1, col2, col3 = st.columns([1, 1, 1])
    with col2:
        try:
            # Usamos a logo vertical 'Marca Águia Florestal-02.png'
            logo_aguia = Image.open("Marca Águia Florestal-02.png")
            st.image(logo_aguia, use_container_width=True)
        except FileNotFoundError:
            st.balloons() # Fallback para balões se a logo não for encontrada
...
# Logo principal (horizontal) no topo
